`catmap` <-
function(dataset, ci, fe.forest, re.forest, printout, fe.sense, fe.senseplot, re.sense, re.senseplot, fe.cumulative, fe.cumplot, re.cumulative, re.cumplot, funnel){
...
study.removed<-paste("Study Removed =", a1$name[f], sep=" ")
...
if(dataset!=catmapdata){
sink(paste(dataset, "fixed.effects.sensitivity.txt", sep="."), append=TRUE)
cat("Fixed Effects Sensitivity Analysis\n")
cat(study.removed, sf.results, sep="\n")
cat("\n")
sink()
...
if (re.sense==TRUE & tau2 > 0){
srplot<-matrix(0,nrow(a1),3)
for(r in 1:nrow(a1)){
sr.weight<-weight[-r]
sr.logOR<-logOR[-r]
sr.comvarlogOR<-comvarlogOR[-r]
...
srstudy.removed<-paste("Study Removed =", a1$name[r], sep=" ")
...
if(dataset!=catmapdata){
sink(paste(dataset, "random.effects.sensitivity.txt", sep="."), append=TRUE)
cat("Random Effects Sensitivity Analysis\n")
cat(srstudy.removed, sr.results, sep="\n")
cat("\n")
sink()
...
cfplot<-matrix(0, nrow(a1),3)
for(c in 1:nrow(a1)){
cf.weight<-weight[1:c]
cf.logOR<-logOR[1:c]
...
study.added<-paste("Study Added =", a1$name[c], sep=" ")
...
if(dataset!=catmapdata){
sink(paste(dataset, "fixed.effects.cumulative.txt", sep="."), append=TRUE)
cat("Fixed Effects Cumulative Meta-Analysis\n")
cat(study.added, cf.results, sep="\n")
cat("\n")
sink()
...
if (re.cumulative==TRUE & tau2 > 0){
crplot<-matrix(0,(nrow(a1)-1),3)
for(u in 2:nrow(a1)){
v<-u-1
cr.weight<-weight[1:u]
cr.logOR<-logOR[1:u]
cr.comvarlogOR<-comvarlogOR[1:u]
...
crstudy.added<-paste("Study Added =", a1$name[u], sep=" ")
...
if(dataset!=catmapdata){
sink(paste(dataset, "random.effects.cumulative.txt", sep="."), append=TRUE)
cat("Random Effects Cumulative Meta-Analysis\n")
cat(crstudy.added, cr.results, sep="\n")
cat("\n")
sink()
...
if(dataset!=catmapdata){
pdf(file=(paste(dataset, "fixed.effects.sensitivity.plot.pdf", sep=".")))
}
if(dataset==catmapdata){
pdf(file=(paste("catmapdata", "fixed.effects.sensitivity.plot.pdf", sep=".")))
}
sfpstudy<-c(1:nrow(a1))
...
sfpstudyname<-paste("Study Removed:",a1$name[z], sep="\n")
...
if (re.senseplot==TRUE & tau2 > 0){
if(dataset!=catmapdata){
pdf(file=(paste(dataset, "random.effects.sensitivity.plot.pdf", sep=".")))
}
if(dataset==catmapdata){
pdf(file=(paste("catmapdata", "random.effects.sensitivity.plot.pdf", sep=".")))
}
srpstudy<-c(1:nrow(a1))
...
srpstudyname<-paste("Study Removed:", a1$name[y], sep="\n")
...
cfpstudyname<-paste("Study Added:", a1$name[x], sep="\n")
...
if (re.cumplot==TRUE & tau2 > 0){
if(dataset!=catmapdata){
pdf(file=(paste(dataset, "random.effects.cumulative.plot.pdf", sep=".")))
}
if(dataset==catmapdata){
pdf(file=(paste("catmapdata", "random.effects.cumulative.plot.pdf", sep=".")))
}
crpstudy<-c(1:(nrow(a1)-1))
...
crpstudyname<-paste("Study Added:", a1$name[w], sep="\n")
...
if (fe.forest==TRUE){
if(dataset!=catmapdata){
pdf(file=(paste(dataset, "fixed.effects.plot.pdf", sep=".")))
}
if(dataset==catmapdata){
pdf(file=(paste("catmapdata", "fixed.effects.plot.pdf", sep=".")))
}
prop.weight<-(weight/(sum(weight))*10)
OR<-c(combinedOR, exp(logOR))
study<-1:(nrow(a1)+1)
lx<-max((min(lbci.fe)-0.25),0)
hx<-max(ubci.fe)+0.25
ly<-min(study)-0.5
hy<-max(study)+0.5
mar1<-c(5.1, 7.1, 4.1, 2.1)
...
par("las"=las1)
par("mar"=mar1)
dummy<-c(rep(NA, length(study)))
dummy[1]<-hx
dummy[2]<-lx
ydummy<-c(rep(NA, length(study)))
ydummy[1]<-hy
ydummy[2]<-ly
xtitle<-paste("OR (", ci100, "% CI)")
plot(dummy, ydummy, type="n", log="x", ylab="", yaxt="n", xlab=xtitle, main="Inverse Variance (Fixed-Effects) ORs")
points(OR[1], study[1], pch=22, cex=1.0, bg="red", col="red")
segments(lbci, 1, ubci, 1, col="red")
mtext("Pooled OR",side=2,at=1, cex=0.80)
abline(v=1.0)
for(i in 2:(nrow(a1)+1)){
j<-i-1
points(OR[i],study[i], pch=22, cex=prop.weight[j], bg="black", col="black")
segments(lbci.fe[j],study[i],ubci.fe[j],study[i], bg="black", col="black")
studyname<-a1$name[j]
mtext(paste(studyname),side=2, at=study[i], cex=0.80)
}
dev.off()
}
...
if (re.forest==TRUE & tau2 > 0){
if(dataset!=catmapdata){
pdf(file=(paste(dataset, "random.effects.plot.pdf", sep=".")))
}
if(dataset==catmapdata){
pdf(file=(paste("catmapdata", "random.effects.plot.pdf", sep=".")))
}
dslprop.weight<-(weight.dsl/(sum(weight.dsl))*100)
dslOR<-c(OR.dsl, exp(logOR))
rstudy<-1:(nrow(a1)+1)
rlx<-max((min(lbci.fe)-0.25),0)
rhx<-max(ubci.fe)+0.25
rly<-min(study)-0.5
rhy<-max(study)+0.5
mar1<-c(5.1, 7.1, 4.1, 2.1)
las1<-1
...
points(dslOR[1], study[1], pch=22, cex=1.0, bg="red", col="red")
segments(lbci.dsl, 1, ubci.dsl, 1, col="red")
mtext("Pooled OR",side=2,at=1, cex=0.80)
abline(v=1.0)
for(i in 2:(nrow(a1)+1)){
j<-i-1
...
if(funnel==TRUE){
if(dataset!=catmapdata){
pdf(file=(paste(dataset, "funnel.plot.pdf", sep=".")))
}
if(dataset==catmapdata){
...
maxse<-max(seLogOR)
minse<-min(seLogOR)
plot(OddsRatio, seLogOR, ylim=rev(c(minse,maxse)),log="x")
abline(v=1)
abline(v=combinedOR, lty=2)
#dev.off()
}
}
